...
class PINN_Loss(nn.Module):
    def __init__(self, xdot_model, alpha=0.5):
        super(PINN_Loss, self).__init__()
        self.xdot_model = xdot_model
        self.alpha = alpha  

    def Evaporator(self, x, u, p):
        ...
        Evaporator = Evap(Ref, Cool_evap)

        return Evaporator._system_dynamics(x, u, p)

    def forward(self, pred_para, ground_truth):
        """
        ...
        p_true, h_ref_out_true = torch.split(ground_truth, 2) # True next time step state variables(Simulink based)
        p_pred, h_ref_out_pred = pred_para[:, :2] # Predicted next time step state variables
        zeta, gamma, eps_tp, eps_sh = pred_para[:, 2:] # Predicted present time step hidden parameters

        balance_losses = []
        for idx in range(len(pred_para)): # detachment is necessary for casadi does not support gpu
            x = (p_pred[idx], h_ref_out_pred[idx])
            u = pred_para[idx]
            p = (zeta[idx], gamma[idx], eps_tp[idx], eps_sh[idx])
            
            dp_dt_mod, dh_dt_mod = self.xdot_model # xdot from torch grad calculation
            dp_dt_bal, dh_dt_bal = Evaporator(x, u, p) # xdot from Mass/Energy Balance Equations
