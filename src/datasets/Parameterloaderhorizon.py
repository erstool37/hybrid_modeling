...
class Parameterloader(Dataset):
    ...
    """
    def __init__(self, dir, sequence_length, method, scaler, horizon):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        step_theta = self.thetas[idx + self.horizon : idx + self.horizon + self.sequence_length- 1] # current time step theta
        pred_state = self.states[idx + self.horizon : idx + self.horizon + self.sequence_length- 1] # next time step state
    ...
    def __len__(self):
        return len(self.states) - self.sequence_length - self.horizon